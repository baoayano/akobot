 ...
 */
export function formatEmoji(emojiId: string, emojiName: string, animated: boolean = false): string {
 ...
 */
export function parseEmoji(emojiString: string): { id: string; name: string; animated: boolean } | null {
 ...
 */
export function formatEmojis(emojis: Array<{ id: string; name: string; animated?: boolean }>): string[] {
  return emojis.map(({ id, name, animated }) => formatEmoji(id, name, animated));
}
